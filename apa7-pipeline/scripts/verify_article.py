...
def extract_parenthetical_keys(text: str) -> list[str]:
    ...
    for match in re.finditer(r"\(([^)]+)\)", text):
        ...
        for part in blob.split(";"):
            ...
            if m:
                fam = re.sub(r"[^A-Za-zÀ-ÿ]", "", m.group(1)).lower()
                keys.append(f"{fam}{m.group(2)}")
    return keys
...
def extract_narrative_keys(text: str) -> list[str]:
    ...
    for match in NARRATIVE_CITATION.finditer(text):
        fam = re.sub(r"[^A-Za-zÀ-ÿ]", "", match.group(1)).lower()
        keys.append(f"{fam}{match.group(2)}")
    return keys
...
def citation_consistency(body_text: str, refs: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    particles = {"de", "da", "do", "dos", "das", "van", "von", "der", "den", "del", "di", "le", "la"}
    alias: dict[str, str] = {}
    for ref in refs:
        ...
        if not ref.get("authors"):
            continue
        family_raw = ref["authors"][0].split(",")[0]
        year = str(ref.get("year") or "nd")
        m = re.search(r"(\d{4})([a-z])?$", key)
        letter = m.group(2) if m and m.group(2) else ""
        compact = re.sub(r"[^A-Za-zÀ-ÿ]", "", family_raw).lower()
        alias[f"{compact}{year}{letter}"] = key
        parts = [p for p in re.split(r"[\s\-‐–—]+", family_raw) if p]
        for part in parts:
            token = re.sub(r"[^A-Za-zÀ-ÿ]", "", part).lower()
            if token and token not in particles:
                alias[f"{token}{year}{letter}"] = key
                alias[f"{token}{year}"] = key
